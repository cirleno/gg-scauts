...
## Versioning / Tags
- Releases are annotated git tags on `main`, one per completed refit/fix batch, always pushed to `origin`. Currently at **v15** (`0884a`, fix: fecha parens desbalanceado em `--sombra-hover` e remove fonte duplicada em `--fonte-titulo`).
